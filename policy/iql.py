...
class IQL:
    def __init__(self, args):
        ...
        self.eval_net = DRQN(self.obs_shape, args=args).to(self.device)
        self.target_net = DRQN(self.obs_shape, args=args).to(self.device)
        if args.load_model:
    ...
    def learn(self, batch, train_step, episode_length):
        ...
        for index in range(episode_length):
            inputs = torch.from_numpy(o[:, index]).to(self.device)
            inputs = inputs.view(-1, self.obs_shape)
            inputs_next = torch.from_numpy(o_next[:, index]).to(self.device)
            inputs_next = inputs_next.view(-1, self.obs_shape)
        ...
        mask = torch.from_numpy(1 - padding).view(num_episode, episode_length, 1).repeat(1, 1, self.n_agents).to(
            self.device)
